...
std::vector<STrack> BYTETracker::update(track_object_t *objects, int len)
{
	...
	// 限制 tracked_stracks 的大小，防止記憶體無限增長
	// 如果 tracked_stracks 超過 1000 個，移除最舊的（按 start_frame 排序）
	const size_t MAX_TRACKED_STracks = 1000;
	if (this->tracked_stracks.size() > MAX_TRACKED_STracks)
	{
		// 按 start_frame 排序，移除最舊的
		std::sort(this->tracked_stracks.begin(), this->tracked_stracks.end(),
			[](const STrack& a, const STrack& b) {
				return a.start_frame < b.start_frame;
			});
		// 移除最舊的 20%
		size_t remove_count = this->tracked_stracks.size() - MAX_TRACKED_STracks;
		for (size_t i = 0; i < remove_count; i++)
		{
			this->tracked_stracks[i].mark_removed();
			removed_stracks.push_back(this->tracked_stracks[i]);
		}
		this->tracked_stracks.erase(this->tracked_stracks.begin(), 
			this->tracked_stracks.begin() + remove_count);
	}
	...
	// 清理 removed_stracks，只保留最近的一些用於去重
	// 限制 removed_stracks 的大小，防止記憶體無限增長
	const size_t MAX_REMOVED_STracks = 500;
	if (this->removed_stracks.size() > MAX_REMOVED_STracks)
	{
		// 移除最舊的 50%
		this->removed_stracks.erase(this->removed_stracks.begin(),
			this->removed_stracks.begin() + (this->removed_stracks.size() - MAX_REMOVED_STracks));
	}
