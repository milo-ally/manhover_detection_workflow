...
// We only care about class 0 (human), but model might have 80 classes (COCO)
...
class Yolo11HumanDetectionModel : public IAIModel {
public:
    int Init(const char* model_path) override {
        printf("[HumanDetection] Loading model: %s\n", model_path);
        confThresh_ = read_env_float("HUMAN_CONF_THRESH", read_env_float("MODEL_CONF_THRESH", CONF_THRESH));
        nmsThresh_ = read_env_float("HUMAN_NMS_THRESH", read_env_float("MODEL_NMS_THRESH", NMS_THRESH));
        printf("[HumanDetection] thresholds: conf=%.3f nms=%.3f\n", confThresh_, nmsThresh_);
    ...
    int Inference(const AX_VIDEO_FRAME_T* pFrame, AI_RESULT_T* pResult) override {
        ...
        for (int i = 0; i < pResult->nObjSize; i++) {
            ...
            pResult->objects[i].class_id = 0; // Force 0 for human
...
private:
    ...
    void generate_proposals(std::vector<Object>& proposals) {
        ...
        for (unsigned int i = 0; i < m_io_info->nOutputSize; ++i) {
            ...
            if (C == 4 * REG_MAX + MODEL_CLASS_NUM) { // Combined Head (64 + 80 = 144)
                heads[H].box = data;
                heads[H].cls = data; // Needs offset
                heads[H].is_combined = true;
            } else if (C == 4 * REG_MAX) { // Box Head (64)
                heads[H].box = data;
            } else if (C == MODEL_CLASS_NUM) { // Cls Head (80)
                heads[H].cls = data;
            }
        }
        ...
        for (auto& pair : heads) {
            ...
            for (int h = 0; h < H; h++) {
                for (int w = 0; w < W; w++) {
                    ...
                    // For Human Detection: We only care about class 0 (Person)
                    // But we should check if it's the max probability to avoid FPs from other classes?
                    // Usually yes.
                    int max_id = 0;
                    ...
                    // Only process if detected class is Person (0)
...
extern "C" {
    IAIModel* CreateAIModel() { return new Yolo11HumanDetectionModel(); }
