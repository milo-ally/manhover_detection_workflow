...
class Yolo11PlateDetectionModel : public IAIModel {
public:
    int Init(const char* model_path) override {
        printf("[PlateDetection] Loading model: %s\n", model_path);
        confThresh_ = read_env_float("PLATE_CONF_THRESH", read_env_float("MODEL_CONF_THRESH", CONF_THRESH));
        nmsThresh_ = read_env_float("PLATE_NMS_THRESH", read_env_float("MODEL_NMS_THRESH", NMS_THRESH));
        printf("[PlateDetection] thresholds: conf=%.3f nms=%.3f\n", confThresh_, nmsThresh_);
...
private:
    ...
    void generate_proposals(std::vector<Object>& proposals) {
        ...
        for (unsigned int i = 0; i < m_io_info->nOutputSize; ++i) {
            ...
            if (C == 4 * REG_MAX + CLASS_NUM) { // Combined Head
                heads[H].box = data;
                heads[H].cls = data; // Needs offset
                heads[H].is_combined = true;
            } else if (C == 4 * REG_MAX) { // Box Head
                heads[H].box = data;
            } else if (C == CLASS_NUM) { // Cls Head
                heads[H].cls = data;
            }
        }
...
extern "C" {
    IAIModel* CreateAIModel() { return new Yolo11PlateDetectionModel(); }
