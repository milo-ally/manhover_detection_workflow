...
#include "../include/osd_renderers/face_detection_osd_renderer.h"
...
bool AIProcessor::loadModel(const std::string& modelPath, const std::string& modelName,
    ...
    if (pluginHint.find("pose") != std::string::npos || 
        ...
        ALOGN("[AIProcessor] Using manhole-cover detection plugin for model: %s", modelPath.c_str());
    ...
    const bool isCrowdPlugin = (pluginPath.find("crowd_plugin") != std::string::npos);
    if (!isCrowdPlugin) {
    ...
    if (!handle) {
        ...
        if (pluginPath != "./libyolo_plugin.so") {
            ALOGW("Trying fallback to libyolo_plugin.so");
            handle = dlopen("./libyolo_plugin.so", RTLD_LAZY);
            if (!handle) {
