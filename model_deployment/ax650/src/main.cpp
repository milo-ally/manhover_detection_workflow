...
int main(int argc, char *argv[]) {
    ...
    // Load label-specific alarm rules, including manhole_cover abnormal states.
    std::string alarmRulesPath = "config/alarm_rules.json";
    if (access(alarmRulesPath.c_str(), F_OK) != 0) alarmRulesPath = "../config/alarm_rules.json";
    if (!AlarmFilter::loadRulesFromFile(alarmRulesPath)) {
        ALOGW("[AlarmFilter] alarm_rules.json unavailable; using built-in fallback rules");
    }
