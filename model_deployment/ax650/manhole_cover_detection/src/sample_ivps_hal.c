...
static void visualize_infer_result(IVPS_RGN_HANDLE handle, SAMPLE_ENGINE_Results *objects, float aspect_w, float aspect_h)
{
...
static AX_VOID SaveFrame(SAMPLE_GRP_T *pThis, int nChn) 
{
    AX_VIDEO_FRAME_T frame;
    int ret = AX_IVPS_GetChnFrame(gSampleGrp.nIvpsGrp, nChn, &frame, -1);
    if (ret != 0)
    {
        printf("AX_IVPS_GetChnFrame(Grp:%d Chn:%d) failed, ret=0x%x.\n", gSampleGrp.nIvpsGrp, nChn, ret);
        return;
    }

    frame.u64VirAddr[0] = (AX_U64)AX_POOL_GetBlockVirAddr(frame.u32BlkId[0]);

    int width = pThis->tPipelineAttr.tFilter[nChn + 1][0].nDstPicWidth;
    int height = pThis->tPipelineAttr.tFilter[nChn + 1][0].nDstPicHeight;
    int format = pThis->tPipelineAttr.tFilter[nChn + 1][0].eDstPicFormat;
    char filename[32];
    int img_size = 0;

    if (format == AX_FORMAT_RGB888) {
        sprintf(filename, "%dx%d.rgb", width, height);
        img_size = width * height * 3;
    } else {
        sprintf(filename, "%dx%d.yuv", width, height);
        img_size = width * height * 3 / 2;
    }

    FILE* fp = fopen(filename, "wb");
    fwrite((void*)frame.u64VirAddr[0], 1, img_size, fp);
    fclose(fp);

    printf("saved frame of chn %d: %s\n", nChn, filename);
}
...
static AX_VOID *IVPS_GetFrameThread(AX_VOID *pArg)
{
    ...
    int chn_num = pThis->tPipelineAttr.nOutChnNum - 1;
    ...
    for (int i = 0; i < chn_num; i++)
    {
        ...
        aspect_w[i] = 3840 * 1.0f / INFER_WIDTH;
        aspect_h[i] = 2160 * 1.0f / INFER_HEIGHT;
