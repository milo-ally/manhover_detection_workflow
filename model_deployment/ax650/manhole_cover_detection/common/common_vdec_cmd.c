...
static void __PrintHelp_Com()
{
    ...
    printf("  --u32RefNum:          reference buffer number. default: 0, \n");
}
...
static AX_S32 __SampleInitOptions(SAMPLE_OPTION_T **ppOptions, SAMPLE_OPTION_NAME_T **ppOptName)
{
    ...
    if (ret) goto ERR_RET_LOG;
    ret = SampleOptionsFill(pOptions, i++, "u32RefNum", '0', AX_TRUE);
    if (ret) goto ERR_RET_LOG;
...
static AX_S32 __VdecCmdParaPrint(SAMPLE_VDEC_CMD_PARAM_T *pstPara, AX_VDEC_GRP VdGrp)
{
    ...
    SAMPLE_LOG("pstPara->u32RefNum:%d", pstPara->u32RefNum);
                                ...
                                AX_VDEC_GRP VdGrp, AX_BOOL bLink)
{
    ...
    if (argc <= 2) {
        if (argc == 2) {
            ptr = argv[1];
            if ((strncmp(ptr, "-h", 2)) || (ptr[2] != 0)) {
                SAMPLE_CRIT_LOG("invalid param:%s", argv[1]);
            }
        }
        __PrintHelp_Com();
        if (bLink)
    ...
    while ((s32Ret = SampleGetOption(argc, argv, options, &prm)) != -1) {
        ...
        case 'M':
            ...
            if ((pstCmd->enInputMode != AX_VDEC_INPUT_MODE_FRAME) &&
                (pstCmd->enInputMode != AX_VDEC_INPUT_MODE_STREAM)) {
                SAMPLE_CRIT_LOG("VdGrp=%d, -M:InputMode:%d error\n",
        ...
        case '0': {
            ...
            else if (strcmp(pPrm->longOpt, "u32RefNum") == 0) {
                pstCmd->u32RefNum = atoi(optarg);
            }
            else {
