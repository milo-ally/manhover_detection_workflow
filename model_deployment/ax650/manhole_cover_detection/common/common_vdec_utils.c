    ...
    if (*ppOutputFile == NULL) {
        ...
        *ppOutputFile = pFileOut;
        ...
        if (pInfo == NULL) {
            ...
            SAMPLE_LOG("VdGrp=%d, VdChn=%d, enImgFormat=%d",
                              pInfo->VdGrp, pInfo->VdChn, pInfo->enImgFormat);
            if (pInfo->bOneShot) {
                ...
                ret = snprintf(pFileOut, AX_VDEC_FILE_NAME_LEN, "group%d_chn%d_format%d_compLevel%d_stride_%d_h_%d.lossy",
                               pInfo->VdGrp, pInfo->VdChn, pInfo->enImgFormat,
                               pInfo->u32CompressLevel, pInfo->u32PicStride, pInfo->u32Height);
            } else {
...
AX_S32 StreamFileParserReadFrame(const SAMPLE_INPUT_FILE_INFO_T *pstBsInfo, SAMPLE_STREAM_BUF_T *pstStreamBuf, size_t *pReadLen)
{
    ...
    uBufSize = pstStreamBuf->uBufSize;
    if (uBufSize < oStreamLen) {
        SAMPLE_CRIT_LOG("uBufSize:0x%x < oStreamLen:0x%lx. bufSize is not enough, "
                        "please increase STREAM_BUFFER_MAX_SIZE",
                        uBufSize, oStreamLen);
        sRet = AX_ERR_VDEC_UNKNOWN;
        goto ERR_RET;
    }
