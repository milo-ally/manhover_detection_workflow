...
AX_S32 COMMON_VIN_GetApdPlateId(AX_CHAR *apd_plate_id)
{
    ...
    if (pFile) {
        ...
        COMM_HW_PRT("fopen /proc/ax_proc/apd_plate_id failed!!!\n");
        return -1;
    }
...
AX_S8 COMMON_ISP_GetBoardId(AX_CHAR *board_id)
{
    FILE *pFile = NULL;
    AX_CHAR temp[BOARD_ID_LEN] = {0};
    ...
    if (pFile) {
        ...
        COMM_HW_PRT("fopen /proc/ax_proc/board_id failed!!!\n");
        return -1;
    }
...
AX_S8 COMMON_ISP_GetSpiDevNode(AX_U8 nDevId)
{
    ...
    COMMON_ISP_GetBoardId(board_id);

    if (!strncmp(board_id, "AX650A_Demo", sizeof("AX650A_Demo") - 1)
        || !strncmp(board_id, "AX650A_EVB", sizeof("AX650A_EVB") - 1)) {
        if (nDevId == 0) {
            return 0;
        } else if (nDevId == 4) {
            return 0;
        }
    } else if (!strncmp(board_id, "AX650N_Demo", sizeof("AX650N_Demo") - 1)
        || !strncmp(board_id, "AX650N_EVB", sizeof("AX650N_EVB") - 1)) {
        if (nDevId == 0) {
            return 0;
        } else if (nDevId == 2) {
            return 0;
