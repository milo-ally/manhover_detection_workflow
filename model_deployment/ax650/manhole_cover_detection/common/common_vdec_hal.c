...
AX_S32 FramePoolInit(AX_VDEC_GRP VdGrp, AX_VDEC_CHN VdChn, AX_U32 FrameSize, AX_POOL *PoolId, AX_U32 u32FrameBufCnt)
{
    ...
    *PoolId = s32PoolId;
    ...
    s32Ret = AX_VDEC_AttachPool(VdGrp, VdChn, s32PoolId);
    if (s32Ret != AX_SUCCESS) {
        AX_POOL_DestroyPool(s32PoolId);
        SAMPLE_CRIT_LOG("Attach pool err. 0x%x\n", s32Ret);
    }

    SAMPLE_LOG_ST("FramePoolInit successfully! %d\n", s32PoolId);

    return s32Ret;
}

AX_S32 VdecStopRecvStream(AX_VDEC_GRP VdGrp)
{
    AX_S32 s32Ret = AX_SUCCESS;
...
AX_S32 VdecStartRecvStream(AX_VDEC_GRP VdGrp)
{
    AX_S32 s32Ret = AX_SUCCESS;
    ...
    if (s32Ret != AX_SUCCESS) {
        ...
        stVdecCtx.GrpStatus[VdGrp] = AX_VDEC_GRP_START_RECV;
    }
...
static AX_S32 __VdecInitUsrPicArgs(SAMPLE_VDEC_FUNC_ARGS_T *pstFuncArgs, SAMPLE_VDEC_USERPIC_T *pstVdecUserPic)
{
    ...
    SAMPLE_LOG("VdGrp=%d, VdChn:%d, --- %d %d %d %d %d %d %s\n", VdGrp, VdChn, pstCmd->bUsrInstant,
...
void *VdecFrameFunc(void *arg)
{
    ...
    SAMPLE_LOG_ST("VDEC Creat Grp %d enDecType %d\n", VdGrp, enDecType);
    ...
    if (pstCmd->tChnCfg[ci].u32PicWidth && pstCmd->tChnCfg[ci].u32PicHeight) {
        tVdecAttr.u32StreamBufSize = pstCmd->tChnCfg[ci].u32PicWidth
                                        * pstCmd->tChnCfg[ci].u32PicHeight * 3 / 2;
    } else {
        tVdecAttr.u32StreamBufSize = BUFFER_SIZE;
    }

    if (tVdecAttr.u32StreamBufSize < MIN_BUFFER_SIZE)
        tVdecAttr.u32StreamBufSize = MIN_BUFFER_SIZE;
    ...
    tStreamBuf.tBufAddr.pVirAddr = malloc(tStreamBuf.uBufSize);

    if (tStreamBuf.tBufAddr.pVirAddr == NULL) {
        SAMPLE_CRIT_LOG("VdGrp=%d, malloc FAILED! uBufSize:%d",
                        VdGrp, tStreamBuf.uBufSize);
        goto ERR_RET_FCLOSE;
    }

    if ((enDecType == PT_H264) && (pstCmd->bResConfig == AX_FALSE)) {
        ...
        for (ci = 0; ci < AX_DEC_MAX_CHN_NUM; ci++) {
            if (pstFunPara->stVdecChnAttr[ci].stAttr.enOutputMode) {
                if (pstFunPara->stVdecChnAttr[ci].stAttr.u32PicWidth) {
                    pstCmd->tChnCfg[ci].u32PicWidth = pstFunPara->stVdecChnAttr[ci].stAttr.u32PicWidth;
                } else {
                    pstCmd->tChnCfg[ci].u32PicWidth = pstCmd->u32MaxPicWidth;
                }

                if (pstFunPara->stVdecChnAttr[ci].stAttr.u32PicHeight) {
                    pstCmd->tChnCfg[ci].u32PicHeight = pstFunPara->stVdecChnAttr[ci].stAttr.u32PicHeight;
                } else {
                    pstCmd->tChnCfg[ci].u32PicHeight = pstCmd->u32MaxPicHeight;
                }

                pstCmd->tChnCfg[ci].enOutputMode = pstFunPara->stVdecChnAttr[ci].stAttr.enOutputMode;
            } else {
                pstCmd->tChnCfg[ci].u32PicWidth = pstCmd->tChnCfg[ci].u32PicWidth ?
                                                  pstCmd->tChnCfg[ci].u32PicWidth : pstCmd->u32MaxPicWidth;
                pstCmd->tChnCfg[ci].u32PicHeight = pstCmd->tChnCfg[ci].u32PicHeight ?
            ...
            SAMPLE_LOG("VdGrp=%d, VdChn=%d, enOutputMode=%d, uWidth=%d-%d-%d, u32PicHeight=%d",
                        VdGrp, ci, pstFunPara->stVdecChnAttr[ci].stAttr.enOutputMode,
                        pstCmd->tChnCfg[ci].u32PicWidth,
                        pstCmd->tChnCfg[ci].u32PicHeight);
        }
    }
    ...
    if (enFrameBufSrc == POOL_SOURCE_USER) {
        ...
        sRet = FramePoolInit(VdGrp, VdChn, FrameSize, &PoolId, u32FrameBufCnt);
        if (sRet != AX_SUCCESS) {
    ...
    if (tStreamBuf.tBufAddr.pVirAddr) {
...
void *_VdecRecvThread(void *arg)
{
    ...
    uStartGrpId = pstCmd->uStartGrpId;
    SAMPLE_LOG_ST("uGrpCount:%d Enter while(1) SelectGrp \n", uGrpCount);
    ...
    while (1) {
        ...
        if (flow_end_total_cnt == uGrpCount) {
            SAMPLE_LOG_ST("flow_end_total_cnt:%d flow end!\n", flow_end_total_cnt);
        }
    }

ERR_RET:

    SAMPLE_LOG_ST(" RecvThread exit, last VdGrp=%d, VdChn:%d\n", VdGrp, VdChn);
    return NULL;
}
...
AX_S32 VdecExitFunc(AX_VDEC_GRP VdGrp)
{
    ...
    if(stVdecCtx.GrpStatus[VdGrp] != AX_VDEC_GRP_DESTROYED) {
        while (1) {
            ...
            stVdecCtx.GrpStatus[VdGrp] = AX_VDEC_GRP_DESTROYED;
            break;
        }
    }
