...
class Yolo11Model : public IAIModel {
public:
    int Init(const char* model_path) override {
        printf("[Yolo11] Loading model: %s\n", model_path);
        confThresh_ = read_env_float("HELMET_CONF_THRESH", read_env_float("MODEL_CONF_THRESH", CONF_THRESH));
        nmsThresh_ = read_env_float("HELMET_NMS_THRESH", read_env_float("MODEL_NMS_THRESH", NMS_THRESH));
        printf("[Helmet] thresholds: conf=%.3f nms=%.3f\n", confThresh_, nmsThresh_);
...
private:
    ...
    void generate_proposals(std::vector<Object>& proposals) {
        for (unsigned int i = 0; i < m_io_info->nOutputSize; ++i) {
            ...
            int W = output.pShape[2];
            int C = output.pShape[3];
            int stride = 640 / H; 

            for (int h = 0; h < H; h++) {
                for (int w = 0; w < W; w++) {
                    float* ptr = data + (h * W + w) * C;
                    float* cls_ptr = ptr + 4 * REG_MAX; 
                    ...
                    float* dfl_ptr = ptr;
                    for (int k = 0; k < 4; k++) {
                        float exp_sum = 0.0f; float weighted_sum = 0.0f;
                        for (int r = 0; r < REG_MAX; r++) {
                            float e = expf(dfl_ptr[k * REG_MAX + r]);
                            exp_sum += e; weighted_sum += e * r;
                        }
...
extern "C" {
    IAIModel* CreateAIModel() { return new Yolo11Model(); }
