...
unsigned int FaceDetectionOSDRenderer::render(const AI_RESULT_T* result,
                                               int dstW, int dstH,
    ...
    const unsigned int rectColor = 0x00FFFF; // 黃色
    ...
    for (unsigned int i = 0; i < result->nObjSize && dispIdx < maxElements - 1; ++i) {
        ...
        std::string labelText = std::string(obj.label) + " " + std::to_string(static_cast<int>(obj.score * 100)) + "%";
        int baseLine = 0;
