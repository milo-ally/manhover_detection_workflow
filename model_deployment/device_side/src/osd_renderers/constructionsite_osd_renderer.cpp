#include "../include/osd_renderers/constructionsite_osd_renderer.h"
#include "opencv2/opencv.hpp"
#include <algorithm>
#include <cstring>

unsigned int ConstructionsiteOSDRenderer::getColorForClass(const char* label) const {
	// 支持帶前綴的標籤（如 [0]person），使用 strstr 檢查是否包含關鍵字
	if (strstr(label, "person") != nullptr) {
		return 0x00FF00; // 綠色
	} else if (strstr(label, "fire") != nullptr) {
		return 0xFF0000; // 紅色
	} else if (strstr(label, "hat") != nullptr) {
		return 0xFFFF00; // 黃色
	} else if (strstr(label, "helmet") != nullptr && strstr(label, "no-helmet") == nullptr) {
		return 0x00FF00; // 綠色
	} else if (strstr(label, "vest") != nullptr) {
		return 0x00FFFF; // 青色
	} else if (strstr(label, "safetyharness") != nullptr) {
		return 0x0000FF; // 藍色
	} else if (strstr(label, "machiney") != nullptr) {
		return 0xFFA500; // 橙色
...
unsigned int ConstructionsiteOSDRenderer::render(const AI_RESULT_T* result,
										   int dstW, int dstH,
	...
	for (unsigned int i = 0; i < result->nObjSize && dispIdx < maxElements - 1; ++i) {
		...
		if (dispIdx < maxElements - 1) {
			...
			memset(textBitmap.get(), 0, textW * textH * 4);
			...
			// RGBA8888：使用 (r, g, b, 255) 以匹配內存佈局
			cv::putText(textMat, labelText, cv::Point(2, labelSize.height + 2),
						cv::FONT_HERSHEY_SIMPLEX, fontScale,
						cv::Scalar(b, g, r, 255), thickness);  // OpenCV 為 BGR
			// 轉成 RGBA8888（IVPS 期望 R,G,B,A）
